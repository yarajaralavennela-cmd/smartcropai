...
.health-score {
    ...
    padding: 20px;
    border-radius: 18px;
    background: linear-gradient(135deg, #e8f5e9, #f7fff5);
...
function showResult(data) {
    ...
    const affectedCount = Number(data.affected_count ?? detections.length);

    let healthMessage = "Needs attention";
    if (healthScore >= 80) {
        healthMessage = "Looking healthy";
    } else if (healthScore >= 50) {
        healthMessage = "Moderate attention needed";
    }

    html += `
        <div class="health-score">
            <div class="health-score-label">
                🌱 AI Estimated Field Health Score
            </div>
            ...
            <div class="health-score-message">
                ${healthMessage}
            </div>

            <div class="health-stats">
                <div class="health-stat">
                    🟢 Healthy<br>${healthyCount}
                </div>

                <div class="health-stat">
                    🔴 Affected<br>${affectedCount}
                </div>

                <div class="health-stat">
                    🔎 Detections<br>${detections.length}
                </div>
            </div>
        </div>
    `;
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        try:
            ...
            return {
                "error": "Invalid image file. Please upload JPG, JPEG or PNG."
            }
        ...
        if len(detections) == 0:
            return {
                "status": "no_detection",
                "message": "No sufficiently confident object was detected.",
                "detections": [],
                "health_score": 100,
                "healthy_count": 0,
                "affected_count": 0,
        ...
        # Prototype Field Health Score:
        # healthy confidence / total confidence.
        # This is an estimate from detected boxes, not a true whole-field survey.
        healthy_weight = sum(
            float(item["confidence"])
            for item in detections
            if is_healthy(item["name"])
        )

        affected_weight = sum(
            float(item["confidence"])
            for item in detections
            if not is_healthy(item["name"])
        )

        total_weight = healthy_weight + affected_weight

        if total_weight > 0:
            health_score = round(
                (healthy_weight / total_weight) * 100,
                1
            )
        else:
            health_score = 0.0
        ...
        affected_count = len(detections) - healthy_count

        best_detection = detections[0]
        best_name = best_detection["name"]
        best_confidence = best_detection["confidence"]

        guidance = get_guidance(best_name)

        return {
            "status": "detected",
            "message": "Crop problem detected.",
            "best_detection": {
                "name": best_name,
